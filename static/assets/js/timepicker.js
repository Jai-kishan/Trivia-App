...
//      url : www.sparrowscripts.com
	...
	var imagePath='/static_media/images/timeimage/';
	...
	function refreshTimePicker(mode) {
		...
		sHTML = "<table><tr><td><table cellpadding=3 cellspacing=0 bgcolor='lightgrey'>";
		for (i=0;i<=11;i++) {
			...
			else if (i!=0 && suffix=="am" ) {
				hr=i;
			}
			else if (i==0 && suffix=="pm" ) {
				hr=12;
			}
			else if (i!=0 && suffix=="pm" ) {
				hr=i+12;
			...
			for (j=0;j<4;j++) {
				sHTML+="<td width=57 style='cursor:hand' onmouseover='this.style.backgroundColor=\"white\"' onmouseout='this.style.backgroundColor=\"\"' onclick='setTimePicker(\""+ hr + ":" + padZero(j*15) + "" + dayn + "\")'><a style='text-decoration:none;color:#000000' href='javascript:setTimePicker(\""+ hr + ":" + padZero(j*15) + "" + dayn + "\")'>" + hr + ":"+padZero(j*15) + "<font color=\"#808080\">" + suffix + "</font></a></td>";
			}
